...
def computer():
    ...
    point_ids_of_periors = []
    for row in data:
        if row[3] == '0':
            point_ids_of_periors.append(int(row[0]))
    ...
    Yi = numpy.matrix([Yi])
    #sigma = 2mm
    Qy = numpy.eye(m)*0.0025+ numpy.diag(numpy.array(Y)*0.000000002)
    dx = numpy.ones((n))
    x = numpy.matrix(priorCoorDataX+priorCoorDataY)
    s = 0
    #adjustment loop
    while numpy.linalg.norm(dx)>0.0000000001:
        ...
        x = numpy.matrix(x)
        #compute dx
        dx = numpy.linalg.pinv(A.transpose()*numpy.linalg.inv(Qy)*A)*A.transpose()*numpy.linalg.inv(Qy)*dy
        x = x.transpose()  + dx
        x = x.transpose()
        s = s + 1
        qxhat = numpy.linalg.pinv(A.transpose()*numpy.linalg.inv(Qy)*A)

    print s
    point_ids_of_periors = numpy.matrix(point_ids_of_periors).transpose()
    ij = numpy.array(distData).transpose()[0:2,:]
    ij = ij.transpose()
    drawer(x.transpose(),qxhat,point_ids_of_periors,ij)
...
B3 = Tkinter.Button(frameBottom, text ="compute",bg='purple', command = computer).pack()
